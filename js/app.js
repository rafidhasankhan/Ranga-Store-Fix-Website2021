...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
      ...
      div.innerHTML = `<div class="single-product modify">
      <div>
    <img class="product-image" src=${image}></img>
    </div>
    <h4>${product.title}</h4>
    <h3>Category: ${product.category}</h3>
    <h4>Price: <span class="price-style"> $ ${product.price} </span></h4>
    <h5>Rating : <span class="rating-style">${product.rating.rate}</span> </h5>
    <h5> <span class="rating-style"> ${product.rating.count} </span> Review </h5>
